...
`index.ts` files under folder directories in `src/` (for example `hooks/`, `utils/`, `context/`) are generated by `pnpm run barrel`. The package root does not publish an `index.ts` export — consumers must import from folder paths such as `@helpwave/hightide-utils/utils`. After adding or renaming util files, run:
