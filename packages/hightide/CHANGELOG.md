...
- Opening the trigger shows a modal titled Select files with remove actions, an add button, and drag-and-drop while the dialog is open
...
- Browser `window` and `document` are read through `SafeGlobals`, which logs and returns `null` when they are missing
- Call sites that can wait until mount now access those globals inside `useEffect` or `useLayoutEffect`
...
- Folder and package entry `index.ts` files are maintained explicitly and committed; the barrel generator is no longer run as part of `init`, `build`, or `clean`
...
- Removed `@/*` TypeScript path mapping, Storybook Vite `@` alias, and Jest `@/` mapper
...
- `VerticalNavigationMenuItem` styling to allow for using the full row when no children exist

## [0.12.14] - 2026-07-01

### Changed

- `SearchBar` styling by moving it to CSS

### Fixed

- `IconButton` removing all classNames after the last change

## [0.12.13] - 2026-07-01

### Changed

- `NavigationCard` now expects a link to define href

## [0.12.12] - 2026-07-01

### Changed

- Update `NavigationCard` and `ActionCard` styling by enlarging the shadow on hover
- Allow replacing the link in the `NavigationCard`
...
- Option on `AppPage` to set the active item or url to open it instantly
...
- Migrated Storybook from `@storybook/nextjs` (Webpack) to `@storybook/nextjs-vite` (Vite)
...
- `initiallyActive` attribute to `TabPanel`s
