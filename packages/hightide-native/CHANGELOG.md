...
- `isProcessing` on `Button` and `IconButton`: presses are ignored while processing, and a loading spinner replaces the icon (or is shown on the left when the button has no icon)
- `ThemedLoadingSpinner` for inline loading indication using the content-theme icon style

## [0.9.1] - 2026-09-03

### Added

- `Button` `textProps` (`Omit<TextProps, 'style'>`) to override label `Text` props such as `numberOfLines` and `ellipsizeMode`

### Changed

- `Button` label text no longer wraps: `numberOfLines={1}` and `ellipsizeMode="tail"` by default
...
- `ChatMessageBubble` (and `ChatAttachmentMessageBubble`) prop `messageStatus` is now `status`
- Depending on the `MessageStatus` sent and sending icons are now differentiated 

## [0.7.1] - 2026-09-01

### Changed

- Folder `index.ts` files are maintained explicitly and committed; the barrel generator is no longer run as part of `init`, `build`, or `clean`
...
- `theme.colors.surfaceInverse` from `@helpwave/hightide-design@0.3.1` (inverted surface color pair)

## [0.3.1] - 2026-08-20

### Fixed

- Switch thumb is vertically and horizontally centered in the track (reads directional `border*Width` after the style adapter no longer sets shorthand `borderWidth`)
...
- Precomputed layout maps on `theme.semantics`: `control`, `container`, and `insideControl`
...
- Element layouts live on `theme.semantics` (`control` / `container` / `insideControl`) instead of `theme.elements`
...
- Root `theme.elements` (use `theme.semantics.control` / `container` / `insideControl`)
