...
- fixed `fileInputTokens` for the text alignment of the menu descriptions
...
- `modalTokenResolver` with `background`, `menu`, and `closeButton` container tokens
- Select and multi-select overlay and menu tokens reuse the modal tokens

## [0.7.2] - 2026-09-01

### Added

- `overlay` theme color for dialog, modal, and bottom-sheet scrims (`color.overlay` on config and resolved theme tokens; `#00000039` in light mode and `#00000060` in dark mode)
- Select and multi-select overlay backgrounds use `themeTokens.color.overlay`

## [0.7.1] - 2026-09-01

### Changed

- Folder `index.ts` files are maintained explicitly and committed; the barrel generator is no longer run as part of `init`, `build`, or `test`
